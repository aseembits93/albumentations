...
class PCA:
    ...
    def inverse_transform(self, x: np.ndarray) -> np.ndarray:
        # Utilizing efficient error message construction and proper exception raising.
        if self.components_ is None or self.mean is None:
            raise ValueError(
                "PCA instance not fitted. Call 'fit' before using this estimator."
            )

        # Return direct transformation result.
