...
def keypoints_d4(
    ...
    **params: Any,
    ...
    """
    # ------------------------------------------------------------------ #
    # Fast dispatch without building temporary dicts / lambdas on
    # every call. The most common ('e') case bails out immediately.
    # ------------------------------------------------------------------ #
    ...
    # These four variants need image dimensions; take them only when needed
    if group_member == "v":  # Vertical flip
        return keypoints_vflip(keypoints, image_shape[0])
    if group_member == "h":  # Horizontal flip
        return keypoints_hflip(keypoints, image_shape[1])
    if group_member == "t":  # Reflect over main diagonal
        return keypoints_transpose(keypoints)
    if group_member == "hvt":  # Reflect over anti‑diagonal
        return keypoints_transpose(
            keypoints_rot90(keypoints, 2, image_shape)
        )
